...
function getDefaultHelp() {
  return [
    ...
    `${_ind1}${_cc.bbk}${_cc.yw}<fep>${_cc.bbk} to set episode progress manually. This`,
    `${_ind1}${_cc.bbk}will force Kitsu to update your progress to ${_cc.yw}<fep>${_cc.bbk}.`,
    '',
    `${_cc.bma}Default Syntax:`,
    `${_ind1}${_cc.byw}aniwatch ${_cc.yw}<name> <ep> <fep>`,
    '',
    `${_cc.bma}Details:${_ind1}`,
    `${_ind1}${_cc.yw}name${_cc.x}   Name of an existing anime on disk`,
    ' ',
    `${_ind1}${_cc.yw}ep${_cc.x}     Episode number of anime ${_cc.yw}<name>${_cc.x} on disk`,
    ' ',
    `${_ind1}${_cc.yw}fep${_cc.x}    ${_cc.bma}(Optional)${_cc.x} Update Kitsu progress with`,
